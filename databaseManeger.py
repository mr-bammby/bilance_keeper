...
class databaseManeger:
    ...
    def insert_transfer(self, sort, amount, day_id, week_id, month_id, year_id, class_id=None, subclass_id=None,
                        subsubclass_id=None, comment=""):
                ...
                try:
                    with sqlite3.connect(self.db_file) as conn:
                        sql = ''' INSERT INTO transfer(account_id,sort,amount,day_id,week_id,month_id,year_id,class_id,subclass_id,subsubclass_id,comment)
                                      VALUES(?,?,?,?,?,?,?,?,?,?,?) '''
                        transfer = (
                        self.account_id, sort, amount, day_id, week_id, month_id, year_id, class_id, subclass_id,
                        subsubclass_id, comment)
                        cur = conn.cursor()
        ...
        if self.account_connected:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM subsubclass WHERE (name=? AND class_id=? AND subclass_id=?)',
                                    (name, class_id, subclass_id))
                        entry = cur.fetchone()
            except Error as e:
                print(e)
        ...
        if self.account_connected:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM months WHERE (month=? AND year_id=?)', (month, year_id))
                        entry = cur.fetchone()
            except Error as e:
                print(e)
        ...
        if self.account_connected:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM weeks WHERE (week=? AND month_id=? AND year_id=?)',
                                    (week, month_id, year_id))
                        entry = cur.fetchone()
            except Error as e:
                print(e)
        ...
        if self.account_connected:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM days WHERE (day=? AND week_id=? AND month_id=? AND year_id=?)',
                                    (day, week_id, month_id, year_id))
                        entry = cur.fetchone()
            except Error as e:
                print(e)
        ...
        if self.account_connected:
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM class WHERE (name=? AND account_id)=?',
                                    (class_name, self.account_id))
                        entry = cur.fetchone()
                        if entry == None:
        ...
        if self.account_connected:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM subclass WHERE (name=? AND class_id=? AND account_id=?)',
                                    (subclass_name, class_id, self.account_id))
                        entry = cur.fetchone()
                        if entry == None:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute(
                            'SELECT * FROM subsubclass WHERE (name=? AND class_id=? AND subclass_id=? AND account_id=?)',
                            (subsubclass_name, class_id, subclass_id, self.account_id))
                        entry = cur.fetchone()
                        if entry == None:
        ...
        if self.account_connected:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM months WHERE (month=? AND year_id=? AND account_id=?)',
                                    (month_name, year_id, self.account_id))
                        entry = cur.fetchone()
                        if entry == None:
        ...
        if self.account_connected:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM weeks WHERE (week=? AND year_id=? AND month_id=? AND account_id=?)',
                                    (week_name, year_id, month_id, self.account_id))
                        entry = cur.fetchone()
                        if entry == None:
        ...
        if self.account_connected:
            try:
                ...
                if type(week_id) is str:
                    temp = self.get_weekID(month_id, year_id, week_id)
                    if type(temp) is int:
                        week_id = temp
                year_id = int(year_id)
            except:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute(
                            'SELECT * FROM days WHERE (day=? AND year_id=? AND month_id=? AND week_id=? AND account_id=?)',
                            (day_name, year_id, month_id, week_id, self.account_id))
                        entry = cur.fetchone()
                        if entry == None:
    ...
    def get_transaction_list(self, type, id):
        if type == "year":
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM transfer WHERE (year_id=?)', (id,))
                        entry = cur.fetchall()
                        if entry == None:
                            return "error year_id"
                        return entry
                    else:
                        return "database_error table_transfers"
            except Error as e:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM transfer WHERE (month_id=?)', (id,))
                        entry = cur.fetchall()
                        if entry == None:
                            return "error month_id"
                        return entry
                    else:
                        return "database_error table_transfers"
            except Error as e:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM transfer WHERE (week_id=?)', (id,))
                        entry = cur.fetchall()
                        if entry == None:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM transfer WHERE (day_id=?)', (id,))
                        entry = cur.fetchall()
                        if entry == None:
                            return "error day_id"
                        return entry
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM transfer WHERE (class_id=?)', (id,))
                        entry = cur.fetchall()
                        if entry == None:
            ...
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM transfer WHERE (subclass_id=?)', (id,))
                        entry = cur.fetchall()
                        if entry == None:
                            return "error subclass_id"
                        return entry
                    else:
                        return "database_error table_transfers"
            except Error as e:
                print(e)
                return "database_error table_transfers"
        elif type == "subsubclass":
            try:
                with sqlite3.connect(self.db_file) as conn:
                    if conn != None:
                        cur = conn.cursor()
                        cur.execute('SELECT * FROM transfer WHERE (subsubclass_id=?)', (id,))
                        entry = cur.fetchall()
                        if entry == None:
                            return "error subsubclass_id"
                        return entry
                    else:
                        return "database_error table_transfers"
            except Error as e:
